...
df <- read.csv("https://hastie.su.domains/ElemStatLearn/datasets/SAheart.data")
y = df$chd
df <- df %>% dplyr::select(-row.names)
df$famhist = ifelse(test = df$famhist == "Present", 1, 0)

View(df) # To see the data

#### Barchart of the data against chd ####
df %>% 
  mutate(across(-chd, as.numeric)) %>% 
  pivot_longer(-chd, names_to = "var", values_to = "value") %>%
  ggplot(aes(x = chd, y = value)) +
  geom_col() +
  facet_wrap(~var, scales = "free") +
  labs(title="Numerical values against chd")
...
for (i in 1:5) {
  ...
  y_train <- train$chd
  y_test <- test$chd
  y_validation <- validation$chd
  
  train  <- df[index == 1, ] %>% dplyr::select(-chd)
  test   <- df[index == 2, ] %>% dplyr::select(-chd)
  validation <- df[index == 3, ] %>% dplyr::select(-chd)
  
  
  #### Adaboost ####
  adaboost_train_fit <- MachineShop::fit(formula = as.factor(chd) ~ ., data = train01, model = "AdaBoostModel")
  ...
  #### Bagging ####
  bagging_train_fit <- ipred::bagging(as.factor(chd) ~ ., data = train01, coob = TRUE)
  ...
  bag_rf_train_fit <- randomForest(as.factor(y_train) ~ ., data = train01)
  ...
  #### Earth model ####
  earth_train_fit <- MachineShop::fit(chd ~ ., data = train01, model = "EarthModel")
  ...
  #### Linear ####
  linear_train_fit <- MachineShop::fit(chd ~ ., data = train01, model = "LMModel")
  ...
  mda_train_fit <- mda::mda(formula = chd ~ ., data = test01)
  ...
  pls_train_fit <- MachineShop::fit(chd ~ ., data = train01, model = "PLSModel")
  ...
  pda_train_fit <- MachineShop::fit(as.factor(chd) ~ ., data = train01, model = "PDAModel")
  ...
  qda_train_fit <- MASS::qda(chd ~ ., data = train01)
  ...
  #### Random Forest ####
  tune_train_rf <- tune.randomForest(x = train, y = as.factor(y_train), data = df, mtry = 1:(ncol(train)-1))
  ...
  #### Ranger ####
  ranger_train_fit <- MachineShop::fit(chd ~ ., data = train01, model = "RangerModel")
  ...
  svm_train_fit <- svm(as.factor(y_train) ~ ., data = train, kernel = "radial", gamma = 1, cost = 1)
  ...
  ensemble1$y <- df[ensemble_row_numbers,"chd"]
  ...
  ensemble_adaboost_train_fit <- MachineShop::fit(formula = as.factor(y) ~ ., data = ensemble_train, model = "AdaBoostModel")
  ...
  ensemble_bag_rf_train_fit <- randomForest(as.factor(ensemble_y_train) ~ ., data = ensemble_train)
  ...
  ensemble_tune_train_rf <- tune.randomForest(x = ensemble_train, y = as.factor(ensemble_y_train), data = ensemble1, mtry = 1:(ncol(train)-1))
  ...
  ensemble_svm_train_fit <- svm(as.factor(y) ~ ., data = ensemble_train, kernel = "radial", gamma = 1, cost = 1)
...
ensemble1 %>% 
  ...
  geom_boxplot() +
  facet_wrap(~var, scales = "free") +
  labs(title="Ensemble boxplot of solution methods vs chd")


results <- data_frame(
  Model = c('ADA Boost', 'Bagging', 'Bagged Random Forest', 'C50', 'Earth', 'Flexible Discriminant Analysis',
            ...
            'Support Vector Machines', 'Ensemble ADA Boost', 'Ensemble Bagging',
            ...
            'Ensemble Regularized Discriminant Analysis', 'Ensemble RPart', 'Ensemble Support Vector Machines'),
  
  'Mean_Train_Accuracy' = round(c(adaboost_train_accuracy_mean, bagging_train_accuracy_mean, 
                                  ...
                                  svm_train_accuracy_mean, ensemble_adaboost_train_accuracy_mean,
                                  ...
                                  ensemble_rda_train_accuracy_mean, ensemble_rpart_train_accuracy_mean, ensemble_svm_train_accuracy_mean),4),
  
  'Mean_Test_Accuracy' = round(c(adaboost_test_accuracy_mean, bagging_test_accuracy_mean,
                                 ...
                                 svm_test_accuracy_mean, ensemble_adaboost_test_accuracy_mean,
                                 ...
                                 ensemble_rda_test_accuracy_mean, ensemble_rpart_test_accuracy_mean, ensemble_svm_test_accuracy_mean),4),
  
  'Mean_Validation_Accuracy' = round(c(adaboost_validation_accuracy_mean, bagging_validation_accuracy_mean,
                                       ...
                                       ensemble_adaboost_validation_accuracy_mean,
                                       ...
                                       ensemble_rda_validation_accuracy_mean, ensemble_rpart_validation_accuracy_mean, ensemble_svm_validation_accuracy_mean),4),
  
  'Mean_Holdout' = round(c(adaboost_holdout_mean, bagging_holdout_mean, bag_rf_holdout_mean, C50_holdout_mean, earth_holdout_mean,
                           ...
                           pda_holdout_mean, rf_holdout_mean, ranger_holdout_mean, rda_holdout_mean, svm_holdout_mean, ensemble_adaboost_holdout_mean, ensemble_bagging_holdout_mean, ensemble_bag_rf_holdout_mean,
                           ...
                           ensemble_ranger_holdout_mean, ensemble_rf_holdout_mean, ensemble_rda_holdout_mean, ensemble_rpart_holdout_mean, ensemble_svm_holdout_mean),4),
  
  'Mean_overfitting' = round(c(adaboost_overfitting_mean, bagging_overfitting_mean, bag_rf_overfitting_mean,
                               ...
                               rda_overfitting_mean, svm_overfitting_mean,
                               ...
                               ensemble_svm_overfitting_mean),4),
  
  'Min_overfitting' = round(c(adaboost_overfitting_range[1], bagging_overfitting_range[1], bag_rf_overfitting_range[1],
                              ...
                              ensemble_adaboost_overfitting_range[1],
                              ...
                              ensemble_rda_overfitting_range[1], ensemble_rpart_overfitting_range[1], ensemble_svm_overfitting_range[1]),4),
  
  'Max_overfitting' = round(c(adaboost_overfitting_range[2], bagging_overfitting_range[2],
                              ...
                              svm_overfitting_range[2], ensemble_adaboost_overfitting_range[2],
                              ...
                              ensemble_svm_overfitting_range[2]),4),
  
  'Sum_of_Diagonal' = round(c(adaboost_table_sum_diag, bagging_table_sum_diag, bag_rf_table_sum_diag, C50_table_sum_diag,
                              ...
                              svm_table_sum_diag, ensemble_adaboost_table_sum_diag,
                              ...
                              ensemble_rf_table_sum_diag, ensemble_rda_table_sum_diag, ensemble_rpart_table_sum_diag, ensemble_svm_table_sum_diag),4),
  
  'Final_fit_train' = c('adaboost_fit_train', 'bagging_fit_train', 'bag_rf_fit_train', 'C50_fit_train', 'earth_fit_train', 'fda_fit_train', 'lda_fit_train',
                        ...
                        'svm_fit_train', 'ensemble_adaboost_fit_train', 'ensemble_bagging_fit_train', 'ensemble_bag_rf_fit_train',
                        ...
                        'ensemble_rf_fit_train', 'ensemble_rda_fit_train', 'ensemble_rpart_fit_train', 'ensemble_svm_fit_train'),
  
  'Summary_Table' = c('adaboost_table', 'bagging_table', 'bag_rf_table', 'C50_table', 'earth_table', 'fda_table', 'lda_table',
                      'linear_table', 'mda_table', 'n_bayes_table', 'qda_table', 'pls_table', 'pda_table', 'rf_table', 'ranger_table', 'rda_table', 
                      'svm_table', 'ensemble_adaboost_table', 'ensemble_bagging_table', 'ensemble_bag_rf_table',
                      ...
                      'ensemble_rf_table', 'ensemble_rda_table', 'ensemble_rpart_table', 'ensemble_svm_table')
)
...
all_tables <- list(adaboost_table, bagging_table, bag_rf_table, C50_table, earth_table, fda_table, lda_table, linear_table,
                   mda_table, n_bayes_table, qda_table, pls_table, pda_table, rf_table, ranger_table, rda_table, svm_table, 
                   ensemble_adaboost_table, ensemble_bagging_table, ensemble_bag_rf_table, ensemble_C50_table, ensemble_earth_table,
                   ensemble_fda_table, ensemble_linear_table, ensemble_n_bayes_table, ensemble_ranger_table, ensemble_rf_table,
                   ensemble_rda_table, ensemble_rpart_table, ensemble_svm_table)
